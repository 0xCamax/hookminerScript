...
contract HookDeployer {
    ...
    function safeDeploy(
        ...
        require(
            ...
            "HookDeployer: Returned wrong address"
        );
    }
    ...
    function hasPermission(address hook, uint160 flag)
        public
        pure
        returns (bool)
    {
    ...
    function getFlag(string memory name) external pure returns (uint160) {
        ...
        if (
            ...
            keccak256("AFTER_ADD_LIQUIDITY_RETURNS_DELTA")
        ) return uint160(1) << 1;
        if (
            keccak256(bytes(name)) ==
            keccak256("AFTER_REMOVE_LIQUIDITY_RETURNS_DELTA")
        ) return uint160(1) << 0;
